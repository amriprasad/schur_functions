...
\begin{example}
  ...
  We visualize the abacus $w$ as a condifuration of beads on a single runner, with possible positions of beads numbered $1, 2, 3, \dotsc$.
  ...
  \begin{displaymath}
    ...
    \end{tabular}
  \end{displaymath}
  The first row shows the positions $k=0,1,\dotsc$ on the runner and the decond row shows the beads.
\end{example}
\begin{theorem}
...
\begin{proof}
  ...
  On the other hand $I(51003204600\dotsb,(0,0,1,0,1,1))=(51002304600\dotsb, (0,1,0,0,1,1))$.
  \end{example}
\end{proof}
...
\begin{displaymath}
  \mathbf S_{(n)} := \sum_{\text{$w\in A_n^*$ is a row}} w
\end{displaymath}
or the elements
\begin{displaymath}
  \mathbf S_{(1^n)} = \sum_{\text{$w\in A_n*$ is a column}} w
\end{displaymath}
...
The algorithm $\rins$ takes as input the single row $t$ and a letter $x$, and returns a pairt $(b, t')$, where $b'$ is either the empty word, or a single letter, and $t'$ is a row:
\begin{displaymath}
...
\begin{displaymath}
  \ins\left(\ytableaushort{13368,245,356,48,3}\right) = \ytableaushort{13338,2456,356,48}.
\end{displaymath}
\end{example}
