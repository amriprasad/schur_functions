...
Schensted's insertion algorithm is a method of inserting a box with the number $x$ into $t$, resulting in a new tableau $t\leftarrow x$, which has remarkable combinatorial properties, which will be discussed in the upcoming sections.

We first consider the case where $t$ has a single row $\ytableaushort{{a_1}{a_2}{\dotsb}{a_k}}$.
For typographical convenience, we do away with the boxes in the tableau.
\begin{displaymath}
  {a_1}{a_2}{\dotsb}{a_k} \leftarrow x =
  \begin{cases}
    \setlength\arraycolsep{0pt}
    \begin{matrix}a_1&a_2&\dotsb&a_k&x\end{matrix} & \text{ if } x\geq a_k,\\
    \\
    \setlength\arraycolsep{0pt}
    \renewcommand{\arraystretch}{0}
    \begin{matrix}{a_j}\\{a_1}&{\dotsb}&{a_{j-1}}&{x}&{a_{j+1}}&{\dotsb}&{a_k}\end{matrix} & \text{ if } j = \min\{k \mid a_k > x\}.
  \end{cases}
\end{displaymath}
