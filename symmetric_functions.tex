...
\begin{theorem}
  ...
  For each $n$, and every integer partition $\lambda$ with at most $n$ parts\footnote{If $\lambda$ has fewer than $n$ parts, it is padded with zeros on the right to make it have exactly $n$ parts}, form the $n\times n$ matrices with $(i,j)$th entry $h_{\lambda_i-i+j}$ and $e_{\lambda_i-i+j}$ respectively.
  Then
  \begin{displaymath}
...
\begin{proof}
  ...
  \begin{figure}
    ...
    \caption{Non-crossing paths corresponding to the tableau $\ytableaushort{23,45,5}$}
  ...
  Figure~\ref{fig:jacobi-trudi} shows the non-crossing path configuration corresponding to $n=5$, $\lambda=(2,2,1,0,0)$ which corresponds to the semistandard tableau $\ytableaushort{23,45,5}$.
  ...
  \begin{proof}
    ...
    Summing over all possible paths gives
    \begin{displaymath}
      \sum_{1\leq i_1<\dotsb <i_j\leq n} x_{i_1}\dotsb x_{i_j} = e_j(x_1,\dotsc,x_n),
    \end{displaymath}
    as required.
  \end{proof}
  \begin{figure}
