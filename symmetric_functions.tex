...
\usepackage{ytableau,graphicx,xypic,hyperref}
\xyoption{all}
...
Give partitions $\lambda=(\lambda_1,\dotsc,\lambda_l)$ and $\mu=(\mu_1,\dotsc,\mu_m)$, if $K_{\lambda\mu}>0$,
...
\begin{proof}
  ...
  In order to undertsand the algorithm that follows, it is helpful to keep in mind Example~\ref{example:can-tab} below.
...
\end{align*}
for $i=1,\dotsc, l$.
Now suppose $\mu = (\mu_1,\dotsc, \mu_l)$ is a partition (possibly padded with zero's so that it has the same number of parts as $\lambda$) such that $\mu_i\subset \lambda_i$, then we can take:
\begin{align*}
  A_i & = (\mu_i+l-i, n),\\
  B_i & = (\lambda_i+l-1, n).
\end{align*}
...
\begin{proof}
  ...
  \end{displaymath}
  Define a monoid homomorphis $\pi_a:L_{a+b}^*\to L_a$ by taking $\pi_a(w)$ to be the word obtained by discarding all letters in $w$ that are not in $\{1,\dotsc,a\}$.
...
\end{proof}
\begin{exercise}
  For each partition $\mu$ with $|\mu|=|\lambda|-|\alpha|$, show that
  Show that
  \begin{displaymath}
...
The arrangement and selection of topics in these notes loosely follows Manivel \cite{manivel}. The use of labelled abaci to prove Schur polynomial identities is detailed in Loehr \cite{loehr}. A nice exposition of the LGV lemma can be found in Viennot's lectures at The Institute of Mathematical Sciences, Chennai in 2016 \cite{imsc2016}. The LGV-lemma proof of the Giambelli identity has been published by Stembridge \cite{stembridge}. The treatment of the RSK correspondence and the Littlewood-Richardson rule is guided by Lascoux and Sch\"utzenberger\cite{ls}, and Lascoux, Leclerc and Thibon \cite{llt}. However, I have not seen the precise definition of $\rsk$ and $\rsk^*$ that I used in these notes published elsewhere. Also my use of integer matrices to prove the Littlewood-Richardson rule does not appear to be widespread. Exercise~\ref{exercise:burge} describes a correspondence introduced by Burge in \cite{burge}. The preceding list leaves out most of the standard references in the area, and should not be regarded as a list of my own readings while preparing  these notes.

Many details of proofs and perpectives emerged in discussions with Digjoy Paul.
I also benefited greatly from informal discussions with Bishal Deb, Sudhir Ghorpade, Sridhar P.~Narayanan, K.~N.~Raghavan, Vijay Ravikumar, Evgeny Smirnov, and  S.~Viswanath, and indeed all the participants of the ATM Workshop on Schubert Varieties, held at The Institute of Mathematical Sciences (see \url{https://www.atmschools.org/2017/atmw/sv}).
