...
\end{exercise}
The method for computing elementary and complete functions from Schur functions illustrated in Examples~\ref{example:expansion-e} and \ref{example:expansion-h} can be expressed as follows:
\begin{theorem}
...
\begin{proof}
  ...
  By the triangularity properties of of Kostka numbers (Section~\ref{sec:triang-kostka-numb}), these identities uniquely determine the Schur functions, therefore $S_\lambda=s_\lambda$ for every partition $\lambda$.
\end{proof}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{skew_jth.png}
  \caption{Non-crossing path configuration corresponding to $\ytableaushort{{}{}3,{}1,{}4,2}$}
  \label{fig:skew-jth}
\end{figure}
...
\end{theorem}
\begin{example}
  The Schur function for $\lambda=(4,4,3,1)=(3,2,0|3,1,0)$ can be computed as:
  \begin{displaymath}
    s_{(3,2,1|3,1,0)} = \det
    \begin{pmatrix}
