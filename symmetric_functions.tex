...
\begin{displaymath}
  \coprod_{\mu/\lambda \text{ is a horiz. strip of size $k$}}\abc(\mu)
\end{displaymath}
...
\end{exercise}
\begin{example}
  The semistandard tableau of shape $(3,2)$ and type $(2,2,1)$ are $\ytableaushort{112,23}$ and $\ytableaushort{113,22}$.
...
Rows and columns are tableaux, with one row and one column respectively.
...
\begin{exercise}
  ...
  \item Show that $tx\equiv \ins(t,x)$. Here tableaux are to be identified with their reading words.
...
Let $S$ be any finite set of points, and $v:S\times S\to R$ be any function (we think of $v$ as a \emph{weight function}).
...
\begin{definition}
  ...
  Given a set $S$ of points, a weight function $v:S\times S\to R$, and a points $A_1,\dotsc,A_n$, and $B_1,\dotsc, B_n$, we say that the \emph{crossing condition} is satisfied if, whenever $1\leq i<j\leq n$ and $1\leq i'<j'\leq n$, and $\omega:A_i \to B_{j'}$ and $\eta: A_j\to B_{i'}$ are paths such that $v(\omega)\neq 0$ and $v(\eta)\neq 0$, then the paths $\omega$ and $\eta$ cross.
\end{definition}
...
\begin{proof}
  ...
  Otherwise, take the least $i$ such that the path $\omega_i=(s_0,\dotsc,s_k)$ crosses another path.
  Let $m$ be the least integer such that $s_m$ lies in another path in the system $\bar\omega$.
  Now choose the least $j$ such that $s_m$ lies in $\omega_j =(t_0,\dotsc,t_l)$.
  Thus we have $i<j$, and $s_m=t_r$ for some $0\leq r\leq l$.
  ...
  \end{align*}
  Clearly, $v(I(\bar\omega)) = v(\bar\omega)$.
  If $\omega_i:A_i\to A_{\sigma(i)}$ and $\omega_j:A_j\to B_{\sigma(j)}$, then $\omega'_i:A_i\to B_{\sigma(j)}$ and $\omega'_j:A_k\to B_{\sigma(i)}$.
  Therefore $\epsilon(I(\bar\omega)) = - \epsilon(\bar\omega)$.
...
\begin{theorem}
  ...
  Then
  \begin{displaymath}
    s_\lambda = \det(h_{\lambda_j+i-j})= \det(e_{\lambda'_j+i-j}).
  \end{displaymath}
\end{theorem}
\begin{proof}
...
\begin{equation}
  \label{eq:skew-jth}
  s_{\lambda/\mu}(x_1,\dotsc,x_n) = \det(h_{\lambda_j-\mu_i+i-j}),
\end{equation}
...
\begin{equation}
  \label{eq:skew-jti}
  s_{\lambda/\mu}(x_1,\dotsc,x_n) = \det(e_{\lambda'_j-\mu'_i+i-j}),
\end{equation}
\begin{exercise}
...
\begin{definition}
  [Yamanouchi Word]
  A word $w\in L_n^*$ is called a Yamanouchi word if $x^u$ is a monomial with weakly decreasing powers for every suffix $u$ of $w$.
\end{definition}
\begin{lemma}
...
\begin{exercise}
  ...
  \end{displaymath}
  Here $K_{\lambda/\alpha,\mu}$ is the number of skew-tableaux of shape $\lambda/\alpha$ and type $\mu$.
\end{exercise}
\subsection{Sources}
