...
Given a tuple $\alpha=(\alpha_1,\dotsc, \alpha_n)$, $x^\alpha$ denotes the monomial $x_1^{\alpha_1}\dotsb x_n^{\alpha_n}$
...
\end{exercise}
\begin{theorem}
The polynomials $m_\lambda(x_1,\dotsc,x_n)$, as $\lambda$ runs over all the integer partition of $d$, form a basis for the space of homegeneous symmetric polynomials of degree $d$ in $n$ variables.
\end{theorem}
...
\end{theorem}
\begin{proof}
  We first prove the second identity involving elemenatry symmetric functions.
  A monomial in the expansion of
  \begin{displaymath}
    e_\lambda = \prod_{i=1}^l \sum_{j_1<\dotsb<j_{\lambda_j}}x_{j_1}\dotsb x_{j_{\lambda_i}} 
  \end{displaymath}
  corresponds to the choice of a summand from each factor.
  Construct a matrix $(a_{ij})$ corresponding to such a choice as follows:
  if the factor $x_{j_1}\dotsb x_{j_{\lambda_i}}$ is chosen from the $i$th factor, then set the entries $a_{i,j_1},\dotsc, a_{i, j_{\lamnda_j}}$ to be $1$.
  Clearly the $i$th row of such a matrix sums to $\lambda_i$.
  The monomial corresponding to this choice is $x^\mu$ if, for each $j$, the the number of $i$ for which $x_j$ appears in $a_{i,j_1},\dotsc, a_{i, j_{\lamnda_j}}$, which is the sum of the $j$th column of the matrix $(a_{ij})$.
